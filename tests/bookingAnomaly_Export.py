...
def test_booking_anomaly_export_api():
    url = 'https://dev-api.pttapp.com/api/booking/anomalies/export'
    headers = {
    ...
    if expected_filename in response.content.decode('latin-1'):
        print("Booking Anomaly exported Successfully")

    assert len(response.content) > 0, "Response content is empty."


test_booking_anomaly_export_api()
