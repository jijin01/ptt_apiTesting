...
def test_bookingPayment_Export_api():
    url = 'https://dev-api.pttapp.com/api/booking/payments/export'
    headers = {
    ...
    if expected_filename in response.content.decode('latin-1'):
        print("Booking Payments exported Successfully")

    assert len(response.content) > 0, "Response content is empty."


test_bookingPayment_Export_api()